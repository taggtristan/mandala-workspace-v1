 ...
 */
const WORKBOOK_ID = 'PASTE_GOOGLE_SHEET_ID_HERE';
...
function doGet() {
  ...
  const payload = {};
  Object.keys(TAB_MAP).forEach(key => payload[key] = getRows(ss, TAB_MAP[key]));
...
function cleanKey(value) {
  return String(value || '').trim().replace(/%/g, 'percent').replace(/[^a-zA-Z0-9]+(.)/g, (_, chr) => chr.toUpperCase()).replace(/^[A-Z]/, chr => chr.toLowerCase());
}
